...
class Nethuns_Sameday_Model_Awb extends Varien_Object {
    ...
    public function __construct(array $params)
    {
        ...
        $this->_setRequest();
        $awb_number = $this->_generateAwb();
        $this->_setTracking($awb_number);
    ...
    protected function _setRequest()
    {
        ...
        $shipping_origin = explode('___', Mage::getStoreConfig('carriers/nethuns_sameday/shipping_origin', $order->getStoreId()));
        $this->_request->setPickupPoint($shipping_origin[0]);
        $this->_request->setContactPerson($shipping_origin[1]);
        $this->_request->setPackageType(Mage::getStoreConfig('carriers/nethuns_sameday/package_type', $order->getStoreId()));
        ...
        $this->_request->setAwbPayment(Mage::getStoreConfig('carriers/nethuns_sameday/awb_payment', $order->getStoreId()));
        $this->_request->setCashOnDelivery($order->getPayment()->getMethod() == 'cashondelivery' ? $order->getGrandTotal() : 0);
    ...
    protected function _setTracking($awb_number)
    {
        if(!$awb_number) {
            return;
        }
